(() => {
  ...
  style.textContent=`
    ...
    .trip-home-control.active{background:#f1f5f2;color:#0b0f0d;border-color:#f1f5f2}.tdh-legend-home-button{width:100%;margin:0 0 16px;padding:15px 16px;border:1px solid #3f7657;border-radius:16px;background:#101a14;color:#eef5f0;text-align:left;font-weight:950}.tdh-legend-home-button span{display:block;color:#72df9e;font-size:10px;letter-spacing:.16em;margin-bottom:4px}.tdh-legend-home-button strong{font-size:16px}
    ...
    .departure-card.tdh-departure-saved{margin-top:14px!important}.travel-navigation-title{display:block;font-size:11px;font-weight:800;letter-spacing:.14em;color:#9aa3ae}
  ...
  function enhance(shell){
    ...
    const removeLegacyPlanning=()=>{travel.querySelectorAll('a,button').forEach(x=>{if(/^view planning$/i.test((x.textContent||'').trim()))x.remove()})};removeLegacyPlanning();
    const travelContent=travel.querySelector('[data-travel-content]');travelContent?.insertAdjacentHTML('afterbegin','<section class="trip-mode-card"><span class="travel-navigation-title">NAVIGATION</span><h2>Let’s roll 🏁</h2><p class="muted">Crew ready. Cars loaded. Pick the next stop and hit the road.</p><div class="travel-actions"><a class="travel-destination" data-waze-stay target="_blank" rel="noopener">Accommodation</a><a class="travel-destination" data-waze-track target="_blank" rel="noopener">Track</a></div></section>');
    const accommodation=state?.accommodation||{},eventName=track;const waze=q=>'https://www.waze.com/ul?&q='+encodeURIComponent(q)+'&navigate=yes';const wa=travel.querySelector('[data-waze-stay]'),wt=travel.querySelector('[data-waze-track]');const stayExtras=document.querySelector('[data-stay-extras]'),arrivalPostcode=stayExtras?.querySelector('[name="postcode"]')?.value||'',stayRoute=arrivalPostcode||accommodation.address||accommodation.location||accommodation.stay_details||'';if(wa){wa.href=waze(stayRoute);wa.classList.toggle('is-unavailable',!stayRoute)}if(wt)wt.href=waze(eventName);
    ...
    let legendHome=homePanel.querySelector('[data-crew-legend-home]');if(!legendHome){legendHome=document.createElement('div');legendHome.dataset.crewLegendHome='1';const status=homePanel.querySelector('.trip-status-box');(status||homePanel.querySelector('.trip-mode-hero'))?.insertAdjacentElement('afterend',legendHome)}legendHome.innerHTML='<button type="button" class="tdh-legend-home-button" data-open-crew-legend><span>TRIP LEGEND 🏆</span><strong>Make your nominations →</strong></button>';let legendPanel=shell.querySelector('[data-trip-panel="crew-legend"]');if(!legendPanel){legendPanel=document.createElement('div');legendPanel.className='trip-panel';legendPanel.dataset.tripPanel='crew-legend';legendPanel.hidden=true;shell.appendChild(legendPanel)}legendHome.querySelector('[data-open-crew-legend]')?.addEventListener('click',()=>{showPanel('crew-legend');window.TDHCrewLegend?.render?.(legendPanel)});
