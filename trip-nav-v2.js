(() => {
  ...
  function enhance(shell){
    ...
    const bingo=document.createElement('div');bingo.className='trip-panel';bingo.dataset.tripPanel='bingo';bingo.hidden=true;shell.appendChild(bingo);const travel=document.createElement('div');travel.className='trip-panel';travel.dataset.tripPanel='travel';travel.hidden=true;travel.innerHTML='<div class="trip-travel" data-travel-content></div>';shell.appendChild(travel);const departure=homePanel.querySelector('.departure-card');if(departure)travel.querySelector('[data-travel-content]').appendChild(departure);const travelContent=travel.querySelector('[data-travel-content]');travelContent?.insertAdjacentHTML('beforeend','<section class="trip-mode-card"><span class="eyebrow">🧭 NAVIGATION</span><h2>On the road</h2><p class="muted">Open the route in Waze when the crew is ready to move.</p><div class="travel-actions"><a class="primary" data-waze-meet target="_blank" rel="noopener">WAZE TO MEET</a><a class="primary" data-waze-track target="_blank" rel="noopener">WAZE TO TRACK</a></div></section>');const accommodation=state?.accommodation||{},eventName=track;const waze=q=>'https://www.waze.com/ul?q='+encodeURIComponent(q)+'&navigate=yes';const wa=travel.querySelector('[data-waze-meet]'),wt=travel.querySelector('[data-waze-track]');if(wa){wa.textContent='WAZE TO ACCOMMODATION';wa.href=waze(accommodation.address||accommodation.location||accommodation.stay_details||'');wa.style.display=(accommodation.address||accommodation.location||accommodation.stay_details)?'':'none'}if(wt)wt.href=waze(eventName);
