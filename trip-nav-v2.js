(() => {
  ...
  style.textContent=`
    ...
    .trip-placeholder{margin-top:6px;text-align:center;padding:34px 20px}.trip-placeholder h2{margin:6px 0 8px}.trip-placeholder p{margin:0;color:#929da8}.trip-travel{display:grid;gap:14px}.travel-actions{display:grid;grid-template-columns:1fr 1fr;gap:9px}.travel-actions a{text-align:center;text-decoration:none}.home-bingo-entry{margin-top:14px;display:flex;justify-content:space-between;align-items:center;gap:12px}.home-bingo-entry h3{margin:4px 0}.home-bingo-entry p{margin:0;color:#929da8;font-size:11px}.home-bingo-entry button{flex:none}
  ...
  function enhance(shell){
    ...
    const travel=document.createElement('div');travel.className='trip-panel';travel.dataset.tripPanel='travel';travel.hidden=true;travel.innerHTML='<div class="trip-travel" data-travel-content></div>';shell.appendChild(travel);const departure=homePanel.querySelector('.departure-card');if(departure)travel.querySelector('[data-travel-content]').appendChild(departure);const travelContent=travel.querySelector('[data-travel-content]');travelContent?.insertAdjacentHTML('beforeend','<section class="trip-mode-card"><span class="eyebrow">🧭 NAVIGATION</span><h2>On the road</h2><p class="muted">Open the route in Waze when the crew is ready to move.</p><div class="travel-actions"><a class="primary" data-waze-meet target="_blank" rel="noopener">WAZE TO MEET</a><a class="primary" data-waze-track target="_blank" rel="noopener">WAZE TO TRACK</a></div></section>');const plan=state?.departurePlan||{},eventName=track;const waze=q=>'https://www.waze.com/ul?q='+encodeURIComponent(q)+'&navigate=yes';const wm=travel.querySelector('[data-waze-meet]'),wt=travel.querySelector('[data-waze-track]');if(wm)wm.href=waze(plan.meet_location||'Cornwall Services');if(wt)wt.href=waze(eventName);homePanel.insertAdjacentHTML('beforeend','<section class="trip-mode-card home-bingo-entry"><div><span class="eyebrow">🎰 BROKEN CAR BINGO</span><h3>Make your prediction</h3><p>Secret until Results. Entries stay open until departure.</p></div><button class="primary" data-open-bingo>PLAY BINGO</button></section>');
    ...
    travelBtn.addEventListener('click',()=>showPanel('travel'));homePanel.querySelector('[data-open-bingo]')?.addEventListener('click',()=>{let bingo=shell.querySelector('[data-trip-panel="bingo"]');if(!bingo){bingo=document.createElement('div');bingo.className='trip-panel';bingo.dataset.tripPanel='bingo';bingo.hidden=true;shell.appendChild(bingo)}showPanel('bingo')});
