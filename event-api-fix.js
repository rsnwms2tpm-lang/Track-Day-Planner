(() => {
  ...
  const OPENTRACK_CAPACITY_BASE = 'https://uehmbzwnbariqebbxcst.supabase.co/functions/v1/opentrack-capacity';
  const baseApi = api;
  ...
  api = async function(action, method = 'GET', body = null) {
    ...
    const [mainResult, meResult, capacityResult, openTrackResult] = await Promise.allSettled([
      ...
      fetch(OPENTRACK_CAPACITY_BASE, opts).then(async r => { const j=await r.json(); if(!r.ok) throw new Error(j.error||`OpenTrack capacity request failed (${r.status})`); return j; })
    ]);
    ...
    const openCaps=openTrackResult.status==='fulfilled'?(openTrackResult.value.events||[]):[];
    const normal=s=>String(s||'').toLowerCase().replace(/\s+(?:full circuit|international|indy|grand prix circuit|gp circuit|300)$/,'').trim();
    const capMap=new Map(caps.map(c=>[`${c.provider}|${c.date}|${normal(c.track)}`,c]));
    openCaps.forEach(c=>capMap.set(`${c.provider}|${c.date}|${normal(c.track)}`,c));
    const combined=[...(main.events||[]),...(me.events||[])].map(e=>{
      const c=capMap.get(`${e.provider}|${e.date}|${normal(e.track)}`);
      return c?{...e,availability:c.availability,spacesAvailable:c.spacesAvailable,availabilityCheckedAt:c.availabilityCheckedAt,availabilitySource:c.availabilitySource||e.availabilitySource}:e;
    });
    const seen=new Set();
    ...
    return {...main,events,sources:[...(main.sources||[]),{provider:'Motorsport Events',ok:meResult.status==='fulfilled',count:me.events?.length||0},{provider:'Javelin live capacity',ok:capacityResult.status==='fulfilled',count:caps.length},{provider:'OpenTrack live capacity',ok:openTrackResult.status==='fulfilled',count:openCaps.length}],source:'Multi-provider live search'};
  };
})();
