(() => {
  ...
  function clickTab(id){const shell=document.querySelector('.trip-mode-shell');if(id==='bingo'){shell?.dispatchEvent(new CustomEvent('tdh-open-bingo'));return}const btn=shell?.querySelector(`[data-trip-tab="${id}"]`);if(!btn||btn.hidden)return;jumping=true;btn.click();setTimeout(()=>{jumping=false},350)}
