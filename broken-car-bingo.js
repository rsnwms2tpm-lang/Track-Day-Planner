(() => {
  ...
  function render(){
    ...
    if(bingo.unlocked&&Array.isArray(bingo.submissionStatus)){const rows=bingo.submissionStatus,entered=rows.filter(x=>x.entered).length;const s=document.createElement('section');s.className='trip-mode-card bingo-submissions';s.innerHTML=`<div class="bingo-submissions-head"><div><span class="eyebrow">CREW STATUS</span><h3>Predictions</h3></div><div class="bingo-submissions-count">${entered} of ${rows.length} entered</div></div><div class="bingo-submission-list">${rows.map(x=>`<div class="bingo-submission"><span>${esc(x.name)}</span><span class="${x.entered?'':'waiting'}">${x.entered?'Entered ✓':'Waiting'}</span></div>`).join('')}</div>`;wrap.appendChild(s)}
