...
function showTrip(tab){window.TDHResultsHome?.close?.();suspendTrack();hidePlanning();const s=tripShell();if(!s)return;s.style.removeProperty('display');document.body.classList.add('trip-mode');if(tab){window.__tdhSetBaseTripTab?.(tab);s.querySelector(`[data-trip-tab="${tab}"]`)?.click()}}
function showPlanning(){window.TDHResultsHome?.close?.();suspendTrack();document.body.classList.remove('trip-mode');const s=tripShell();if(s)s.style.setProperty('display','none','important');const p=planning();if(p)p.style.removeProperty('display')}
async function showEvent(){window.TDHResultsHome?.close?.();hidePlanning();document.getElementById('tdhPostTrack')?.remove();const s=tripShell();if(s)s.style.removeProperty('display');document.body.classList.add('trip-mode');await window.TDHTrackDayMode?.open?.()}
async function showResults(){suspendTrack();hidePlanning();document.getElementById('tdhPostTrack')?.remove();const s=tripShell();if(s)s.style.removeProperty('display');document.body.classList.add('trip-mode');await window.TDHResultsHome?.open?.()}
