...
function showTrip(tab){leaveHome();window.TDHPreEvent?.close?.();window.TDHResultsHome?.close?.();suspendTrack();hidePlanning();const s=tripShell();if(!s)return false;s.style.removeProperty('display');document.body.classList.add('trip-mode');if(tab){window.__tdhTripPanel=tab;window.__tdhSetBaseTripTab?.(tab);if(typeof s.__tdhShowTripPanel==='function')s.__tdhShowTripPanel(tab);else s.querySelector(`[data-trip-tab="${tab}"]`)?.click()}return true}
